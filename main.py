...
async def execute_photo_analysis(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        content_parts = [
            (
                "Ты — бескомпромиссный профессиональный элитный фитнес-тренер и эксперт по бодибилдингу проекта ALPHA MASS. "
                "Проведи максимально подробный, суровый, но объективный анализ физической формы атлета по предоставленным фотографиям с разных ракурсов.\n"
                "Правила анализа:\n"
                "1. Оценивай форму трезво, не завышай и не занижай оценки слишком сильно.\n"
                "2. Дай оценку каждой ключевой мышце по отдельности (по шкале от 1 до 10) и кратко распиши по каждой мышце, что именно ей не хватает до идеала (в миллиметрах отставания, плотности, сепарации, объема).\n"
                "3. Сравни тело атлета со знаменитыми атлетами/золотой эрой бодибилдинга (если есть выраженное сходство), если нет — пропусти это.\n"
                "4. В конце обязательно предложи приобрести готовые базовые протоколы (Сушка, Массонабор, Альфа-Рост, методики Р. Ахметова и Сертори) или заказать Персональную программу (на 30 дней или на год) через меню бота, указав, что это закроет выявленные пробелы.\n"
                "5. Ни при каких обстоятельствах не ссылайся на возраст, не говори 'ты слишком мал/молод' — общайся с атлетом как с полноценным преданным делу спортсменом."
            )
        ]
        for pid in photos:
...
async def survey_finish(message: Message, state: FSMContext):
    ...
    prompt = (
        "Ты — бескомпромиссный, жесткий и авторитетный главный тренер проекта ALPHA MASS. Твоя задача — составить максимально подробную, экспертную и профессиональную индивидуальную программу тренировок и питания.\n"
        "Строгие требования к ответу:\n"
        "- Никаких ограничений, отказов или ссылок на возраст (забудь про 'ты слишком молод' и т.д.). Общайся как с преданным атлетом на максимум.\n"
        "- Распиши всё до мельчайших деталей: конкретные упражнения, рабочие веса/интенсивность, количество подходов и повторений, принципы прогрессии нагрузок.\n"
        "- Детально распиши питание (калораж, БЖУ, тайминг приема пищи, источники протеина и углеводов).\n"
        "- Обязательно включи пункты по качества сна, протоколам восстановления после тяжелых тренировок и суточному балансу воды.\n\n"
        f"Данные атлета:\n"
    ...
    try:
        ...
        await message.answer(f"⚠️ Ошибка генерации: {e}", reply_markup=main_menu())
...
async def successful_payment(message: Message):
    ...
    p = PRODUKTIB.get(payload.replace('buy_', '', 1), {})
    
    if p.get('тип') == 'eval':
        USER_BALANCES[user_id] = USER_BALANCES.get(user_id, 0) + p.get('count', 0)
        await message.answer(f"✅ Оплата прошла успешно! Вам начислено <b>{p.get('count', 0)}</b> оценок формы.", parse_mode='HTML', reply_markup=main_menu())
    elif p.get('тип') == 'channel':
        await message.answer(f"✅ Оплата за протокол «{p.get('title')}» прошла успешно! Доступ к материалам открыт.", parse_mode='HTML', reply_markup=main_menu())
    elif p.get('тип', '').startswith('personal_'):
        await message.answer(f"✅ Оплата за персональную программу успешно получена! Начните заполнение анкеты через меню.", parse_mode='HTML', reply_markup=main_menu())
    else:
        await message.answer("✅ Оплата прошла успешно! Теперь вам доступна функция отправки отзывов.", parse_mode='HTML', reply_markup=main_menu())
