...
PRODUKTIB = {
    "сухой" : { "title" : "Сушка", "цена" : 49, "канал" : - 1004395381148, "type" : "channel" },
    "масса" : { "title" : "Массонабор", "цена" : 49, "канал" : - 1004372480639, "type" : "channel" },
    "rost" : { "title" : "Альфа-Рост", "цена" : 99, "канал" : - 100442056365, "type" : "channel" },
    "rustam" : { "title" : "Методика Р. Ахметова", "цена" : 99, "канал" : - 1004342066932, "type" : "channel" },
    "сертори" : { "title" : "Методика Сертори", "цена" : 99, "канал" : - 1003701087960, "type" : "channel" },
...
def basic_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Массонабор – 49 ⭐", callback_data="buy_mass")],
        [InlineKeyboardButton(text="Альфа-Рост – 99 ⭐", callback_data="buy_rost")],
        [InlineKeyboardButton(text="Методика Сертори – 99 ⭐", callback_data="buy_sertori")],
...
async def successful_payment_handler(message: Message, bot: Bot):
    ...
    # Если купили персональную программу (30 дней или год)
    if product.get("type") == "personal":
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💬 Написать автору", url=f"https://t.me/{ADMIN_USERNAME}")]
        ])
        await message.answer(
            f"✅ <b>Оплата успешно получена!</b> \n\n"
            f"🔥 Вы приобрели: <b>{product['title']}</b>\n\n"
            f"Теперь нажмите на кнопку ниже, чтобы написать мне в личные сообщения и начать работу!",
            reply_markup=keyboard, parse_mode="HTML"
        )
    else:
        # Для базовых протоколов — выдаем одноразовую ссылку на закрытый канал
        try:
