...
CHANNEL_USERNAME = "@alphamasss"  # Официальный канал для проверки подписки

gemini_client = genai.Client()
dp = Dispatcher()

# Базы данных в памяти
USER_BALANCES = {} # Баланс оценок формы {user_id: count}
USER_PHOTOS = {}   # Фото для текущей оценки формы {user_id: [file_id, ...]}

# Продукты для покупки через Stars (включая методики и пакеты оценок)
PRODUKTIB = {
    'сухой': {
        'title': 'Сушка',
        'цена': 49,
        'канал': -1004395381148, # ID закрытого канала для выдачи ссылки
        'type': 'channel',
    },
    'масса': {
        'title': 'Массонабор',
...
# --- 5. Проверка подписки по юзернейму ---
...
async def start_handler(message: Message, bot: Bot):
    ...
    if user_id not in USER_BALANCES:
        USER_BALANCES[user_id] = 5  # Даем 5 бесплатных оценок на старте
...
# --- 8. ОЦЕНКА ФОРМЫ (до 5 ракурсов за 1 оценку + продажа услуг) ---
...
async def execute_photo_analysis(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    prompt = (
        ...
        "Учти абсолютно все мышцы (плечи, грудные, спина, руки, пресс, квадрицепсы, икры, пропорции). "
        "Структура ответа:\n"
        "1. Общий вердикт и уровень (процент жира, сильные и слабые мышечные группы).\n"
        "2. Детальный разбор по зонам и ракурсам.\n"
        "3. Что конкретно стоит сделать ради идеальной формы (подсушиться, набрать массу, скорректировать пропорции).\n"
        "4. Точная рекомендация актуальных платных программ и методик из нашего каталога, которые идеально решат его проблемы (предложи конкретно: «Сушка», «Массонабор», «Альфа-Рост», «Методика Р. Ахметова», «Методика Сертори», «Программа на 30 дней» или «Программа на год»). Объясни почему.\n"
        "Пиши профессионально, уверенно, развернуто и убедительно."
    )
    ...
    try:
        ...
        # Кнопки для покупки услуг, которые посоветовал ИИ
        ...
        USER_BALANCES[user_id] += 1  # Возвращаем оценку обратно при сбое
        await bot.send_message(user_id, "⚠️ Произошла ошибка при анализе фотографий. Ваша оценка возвращена на баланс.", reply_markup=main_menu())

# --- 9. Анкетирование для персональных программ (30 дней / Год) ---
...
async def survey_cond(message: Message, state: FSMContext):
    ...
    await message.answer("📋 <b>Шаг 7/8</b>\n\nОграничения / Травмы (укажите травмы или «нет»):")
...
async def survey_inj(message: Message, state: FSMContext):
    ...
    await message.answer("📋 <b>Шаг 8/8</b>\n\nДоступный спортпит и добавки (протеин, креатин или «только еда»):")
...
async def survey_finish_and_generate(message: Message, state: FSMContext):
    ...
    await message.answer("⏳ Элитный ИИ-тренер составляет вашу персональную экспертную программу... Пожалуйста, подождите.")

    if prog_type == '30_days':
        prompt = f"""
Ты — профессиональный элитный фитнес-тренер и спортивный нутрициолог с 10-летним стажем. Твоя задача — составить максимально эффективную, детальную и готовую к работе МЕСЯЧНУЮ программу тренировок, питания и восстановления на 30 дней (натуральный тренинг, строго без химии) для конкретного человека.
...
- Опыт тренировок: {data.get('experience')}
- Условия тренировок: {data.get('conditions')}
- Ограничения / Травмы: {data.get('injuries')}
- Доступный спортпит и добавки: {data.get('supplements')}

Составь четкий план на 4 недели (30 дней), разделив месяц на 4 этапа.
Структура ответа:
1. Стратегия месяца: Краткая задача на эти 30 дней.
2. Расписание тренировок по неделям: Дни, упражнения, подходы, повторения, рабочие веса, отдых.
3. План питания и КБЖУ: Ориентиры по калориям и белкам, пример рациона.
4. Как принимать спортпит.
5. Восстановление (сон, вода).
Пиши четко, структурировано, без «воды», только сухая практика.
"""
    else:
        prompt = f"""
Ты — профессиональный элитный фитнес-тренер и спортивный нутрициолог с 10-летним стажем. Твоя задача — составить максимально эффективную, безопасную и детализированную ГОДОВУЮ программу тренировок, питания и восстановления (натуральный тренинг, строго без химии) для конкретного человека.
...
Составь годовую программу, разбив её на 4 тренировочных цикла (по 3 месяца каждый).
Структура ответа:
1. Стратегия года: Краткий план смены фаз за 12 месяцев.
2. Фаза 1 (Месяцы 1-3).
3. Фаза 2 (Месяцы 4-6).
4. Фаза 3 (Месяцы 7-9).
5. Фаза 4 (Месяцы 10-12).
6. Питание, спортпит и восстановление.
Пиши четко, профессионально, без «воды», только база и биомеханика.
"""

    try:
        response = gemini_client.models.generate_content(model='gemini-2.5-flash', contents=prompt)
        await message.answer(f"👑 <b>ПЕРСОНАЛЬНАЯ ПРОГРАММА ГОТОВА</b>\n\n{response.text}", reply_markup=main_menu(), parse_mode='HTML')
    except Exception as e:
        logging.error(f"Ошибка генерации программы: {e}")
        await message.answer("⚠️ Произошла ошибка при генерации программы. Попробуйте позже.", reply_markup=main_menu())

# --- 10. Покупка товаров и инвойсы Telegram Stars ---
@dp.callback_query(F.data.startswith('buy_'))
async def product_handler(callback: CallbackQuery):
  payload = callback.data.replace('buy_', '', 1)
  product = PRODUKTIB.get(payload)
  if not product:
  ...
  await callback.message.answer_invoice(
      title=product['title'],
      description=f"Оплата услуги: {product['title']
