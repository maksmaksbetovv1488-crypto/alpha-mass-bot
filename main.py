...
BOT_TOKEN = '8863657412:AAGaijX3MyBV8YLKhjaQcAUMcymTxr5LIWo'
CHANNEL_USERNAME = "@alphamasss"
ADMIN_ID = 7847949636

bot = Bot(token=BOT_TOKEN)

gemini_client = genai.Client(api_key="AIzaSyAb8RN6LhCLsZeVmkAvhoSTP76uXdBHJFrQlrnzFFMhRnIuedog")
...
async def start_form_eval(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Отправьте <b>до 5 фотографий</b> вашей формы с разных ракурсов (спереди, сзади, сбоку и т.д.).\n"
        "Когда скинете все нужные ракурсы, нажмите кнопку <b>«✅ Готово к анализу»</b>.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def admin_broadcast_prompt(message: Message, state: FSMContext):
    ...
    await message.answer("✍️ Отправьте или перешлите сообщение для рассылки.")
...
async def successful_payment_handler(message: Message, state: FSMContext):
    ...
    product = PRODUKTIB.get(payload, {})
    product_name = product.get("title", "Товар")
    user_id = message.from_user.id
    
    review_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✍️ Оставить отзыв о покупке", callback_data="leave_review")]
    ])

    if product.get('type') == 'eval':
        count = product.get('count', 0)
        if user_id not in USER_BALANCES:
            USER_BALANCES[user_id] = 0
        USER_BALANCES[user_id] += count
        await message.answer(
            f"✅ <b>Оплата прошла успешно!</b>\n\n"
            f"Вам начислено оценок: +{count} 💎\n"
            f"Текущий баланс: {USER_BALANCES[user_id]} оценок.",
        ...
        await message.answer(
            f"✅ <b>Оплата прошла успешно!</b>\n\n"
            f"Спасибо за покупку: <b>{product_name}</b>.\n"
            f"Транзакция ID: <code>{payment_info.telegram_payment_charge_id}</code>",
            reply_markup=review_keyboard,
            parse_mode="HTML"
        )
