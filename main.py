...
import os
from config import USERS_FILE, REVIEWS_FILE, REFERRALS_FILE
...
from config import bot, dp, ADMIN_ID, CHANNEL_USERNAME, gemini_client, PRODUKTIB, USER_BALANCES, USER_PHOTOS
from database import add_user, get_all_users, save_review, get_all_reviews, save_referral_link, get_referrer, mark_referral_rewarded, is_referral_rewarded
from states import ProgramStates, FormStates, BroadcastState, ReviewState
from keyboards import get_sub_keyboard, main_menu, admin_keyboard, basic_keyboard, personal_keyboard, shop_eval_keyboard
...
async def execute_photo_analysis(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        response = gemini_client.models.generate_content(model='gemini-2.0-flash', contents=content_parts)
...
async def successful_payment(message: Message):
    ...
    await message.answer("✅ Оплата прошла успешно!", reply_markup=main_menu())
